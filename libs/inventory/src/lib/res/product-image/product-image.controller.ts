...
@ApiTags(ProductImageEntityName + 'Controller')
@Controller()
export class ProductImageController {
  constructor(private readonly service: ProductImageService) {}

  @Rest.Get()
  findAll(@Query(ValidationPipe) query: QueryProductImageDto) {
  ...
  @Rest.Post()
  save(@Body(ValidationPipe) body: CreateProductImageDto) {
  ...
  update(
    @ParamId() id: number,
    @Body(ValidationPipe) body: UpdateProductImageDto
  ) {
